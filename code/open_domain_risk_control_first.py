...
parser.add_argument('--alpha', type=list, default=[0.05], help='risk level')
    ...
    for epoch in range(args.multi_check):
        ...
        for s in range(1, args.num_generations_per_prompt + 1):
            ...
            if r_upper <= alpha:
                ...
                if s == args.num_generations_per_prompt:
        ...
        if optimal_s is not None:
            ...
            base_miscoverage_count = 0
            for test_data in test_set:
                id = test_data['id']
                relevance_score_for_correctness = id_to_similarity[id]
                test_miscoverage_count += int(max(relevance_score_for_correctness[:optimal_s]) < args.relevance_threshold)
                base_miscoverage_count += int(max(relevance_score_for_correctness) < args.relevance_threshold)
            error_rate = test_miscoverage_count / num_test
            base_error_rate = base_miscoverage_count / num_test
            print(error_rate)

            multi_check_emr_list.append(error_rate)
            base_error_list.append(base_error_rate)
    mean_base_error_rate = np.mean(np.array(base_error_list))
    mean_base_error_rate_list.append(mean_base_error_rate)
...
print(mean_base_error_rate_list)
