...
# ----------------------------------------------------------------------------------------------------------------------
sampling_size_list = []
prediction_set_size_list = []
beta_list = args.beta
for beta in beta_list:
    multi_check_ss_list = []
    multi_check_apss_list = []
        ...
        # assert optimal_s is not None
            ...
            if t_hat is None:
                ...
                for test_data in test_set:
                    ...
                    uncertainty = np.array(id_to_uncertainty[id][:optimal_s])
                    relevance_score_for_correctness = np.array(id_to_similarity_scores[id][:optimal_s])
                    set_size = len(relevance_score_for_correctness[uncertainty <= t_hat].tolist())
                    set_size_list.append(set_size)
                apss = np.mean(set_size_list)

                multi_check_ss_list.append(optimal_s)
                multi_check_apss_list.append(apss)
    ss = np.mean(multi_check_ss_list)
    pss = np.mean(multi_check_apss_list)

    sampling_size_list.append(ss)
    prediction_set_size_list.append(pss)

print(args.beta)
print(sampling_size_list)
print(prediction_set_size_list)