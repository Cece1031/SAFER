...
# ----------------------------------------------------------------------------------------------------------------------
# for rouge-L score
parser.add_argument('--rouge-path', type=str, default=r'<PATH_TO_METRIC_CACHE>\rouge.py',
                    help='local path of rougeL-score')
# ----------------------------------------------------------------------------------------------------------------------
...
# for rouge-L score
rouge = evaluate.load(args.rouge_path)
# ----------------------------------------------------------------------------------------------------------------------
...
for generation in tqdm.tqdm(generations):
    ...
    #         elif j > i:
    #             similarity_dict[id][i].append(rouge.compute(predictions=[gen_temp], references=[gen])['rougeL'])
    #         elif j < i:
    #             similarity_dict[id][i].append(similarity_dict[id][j][i])
    #     print(f'id-{id} --- rouge-L score-{i}: ', similarity_dict[id][i])
    # ------------------------------------------------------------------------------------------------------------------
    similarity_to_gt_list = []
    for sampled_gen in sampled_generated_texts:
        qa_1 = question + ' ' + gt_answer
        qa_2 = question + ' ' + sampled_gen
        similarity_to_gt_list.append(rouge.compute(predictions=[sampled_gen], references=[gt_answer])['rougeL'])
    similarity_for_correctness[id] = similarity_to_gt_list
    print(f'id-{id} --- rouge-L score for correctness: ', similarity_for_correctness[id])
...
# save
with open(f'{run_name}/rougel_scores.pkl', 'wb') as record_file:
    pickle.dump(results, record_file)
print('Record saved to ', f'{run_name}/rougel_scores.pkl')
