...
# ----------------------------------------------------------------------------------------------------------------------
# load row data (download from hugging face https://huggingface.co/datasets/allenai/sciq/tree/main/data)
...
cleaned_data = []
for sample in tqdm.tqdm(dict_list):
    answer_ids = generative_tokenizer.encode(sample['correct_answer'])
    if len(answer_ids) <= 36:
        cleaned_data.append(sample)
print('Cleaned num: ', len(cleaned_data))
dict_list = cleaned_data
# ----------------------------------------------------------------------------------------------------------------------
# prompt engineer
...
for idx in idx_for_few_shot_prompt:
    ...
    answer = dict_list[idx]['correct_answer']
    ...
    if sample_idx not in idx_for_few_shot_prompt:
        ...
        if question.isascii() and answer.isascii():
            ...
            if len(input_ids) < max_input_ids_length:
                ...
                dataset['id'].append('id_' + str(sample_idx))
...
dataset.save_to_disk(f'{args.data_dir}/sciqa_{model_name}')
# ----------------------------------------------------------------------------------------------------------------------
# check answer length
len_list = []
